...
beforeEach(() => {
  vi.clearAllMocks();
  process.env = { ...ORIGINAL_ENV, NEXT_PUBLIC_APP_URL: "https://app.pinavia.io" };
});
...
describe("Google connector OAuth configuration", () => {
  it("treats blank Google credentials as unconfigured before redirecting to Google Drive OAuth", async () => {
    ...
    const res = await installGoogleDrive(new Request("https://app.pinavia.io/api/connectors/google-drive/install"));
  ...
  it("treats blank Google secrets as unconfigured before redirecting to Gmail OAuth", async () => {
    ...
    const res = await installGmail(new Request("https://app.pinavia.io/api/connectors/gmail/install"));
  ...
  it("trims Google credentials before constructing provider URLs", () => {
    ...
    expect(driveUrl.searchParams.get("redirect_uri")).toBe(
      "https://app.pinavia.io/api/connectors/google-drive/callback"
    );
    expect(gmailUrl.searchParams.get("redirect_uri")).toBe("https://app.pinavia.io/api/connectors/gmail/callback");
  });
});
