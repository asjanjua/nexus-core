...
describe("engagement scope stays out of practice management", () => {
  ...
  it("does not use a migration number the sibling branch also claims", () => {
    // feat/vantage-deep-routes adds 0055 and 0056 concurrently. Two branches
    // claiming one number is a conflict that resolves silently under pressure.
    expect(existsSync(join(process.cwd(), "db/migrations/0055_vantage_deals.sql"))).toBe(false);
