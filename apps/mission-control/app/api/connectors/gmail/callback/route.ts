 ...
 * to their Gmail mailbox (read-only).
 *
 * Flow:
...
export async function GET(request: Request) {
  ...
  const statePayload = await consumeConnectorCallbackState(state, "gmail");
  if (!statePayload) {
  ...
  return redirectWithConnectorInstalled(appUrl, "Gmail");
}
