...
- Diagnostic intake honesty pass in progress: `/diagnostic` no longer claims an unwired paid checkout. The primary action is `Start diagnostic intake` to `/start-pilot?intent=diagnostic`, the secondary action remains `/readiness`, diagnostic pricing is hidden until checkout/receipt flows exist, and `/start-pilot` acknowledges diagnostic intent in the hero. Figma V0.7 frame `Launch Cockpit / 08 Diagnostic Intake Path / 1440` (`145:2`) was added and screenshot-verified after fixing route-card overflow. Local TypeScript, production build, and desktop/mobile route smoke passed; push, deploy, and live smoke remain next.
