...
- Added Figma page `24 Initial Launch Cockpit V0.7` (`https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=129-6`) with four 1440x900 frames: executive launch status, demo route map, final screen QA matrix, and action board. Screenshot validation found one header/subtitle overlap in the QA matrix; it was fixed and re-rendered cleanly.
- V0.7 is the colleague-review start page. It consolidates V0.4/V0.5/V0.6 rather than replacing them: use it to choose what to show, where the source screens live, and which claims remain blocked by DNS/auth/deep-route gates.
- Meridian code polish after V0.7: `components/meridian-submission-panel.tsx` now shows user inputs and action points on each planned deep-route card, labels them `planned deep route`, and adds global jurisdiction-pack requirements beside the no-filing/no-certification boundaries. Focused Meridian registry tests, standalone TypeScript, and production build passed.
- Added V0.7 frame `Launch Cockpit / 05 Pilot Start Intake / 1440` (`135:2`) after `/start-pilot` became a real public route. The frame was screenshot-verified after fixing a wrapped title and narrow sign-in pill.
- `/start-pilot` now renders a public Pinavia pilot-start intake page instead of redirecting immediately to signup. It keeps `Create pilot workspace` as the primary CTA, links diagnostic/email as secondary actions, lists kickoff inputs, branches to product-room previews, and states the no-approval/no-signing/no-filing/no-certification/no-submission/no-investment-decision boundary. Deployed through `1b8c5f4`; live apex smoke returns 200 and content checks confirm the Pinavia shell, headline, and workspace CTA.
- Follow-up landing CTA alignment: homepage hero, public header, money-map CTA, competitive-difference CTA, footer, and final pilot CTA now converge on `/start-pilot`; readiness remains secondary. Product-family cards now link Meridian, Vantage, and Nucleus to their live protected hubs. Figma frame `135:2` was updated and screenshot-verified with the homepage-to-start-pilot handoff. Deployed through `90b911a`; live `/` smoke confirms no `Design candidate` copy remains, six links point to `/start-pilot`, and repeated Meridian/Vantage/Nucleus links point to `/meridian`, `/vantage`, and `/nucleus`.
- Product brief refresh deployed: `/product-brief` has been rewritten from old NexusAI-only collateral into a Pinavia family pilot brief with current room routes, buyer/workflow/proof/boundary rows, mobile product-room cards, pilot inputs/outputs, trust claims, and Start pilot/diagnostic follow-up. Figma V0.7 frame `Launch Cockpit / 06 Product Brief / 1440` (`141:2`) was added and updated to show commit `4e8b71f` live-smoked. Live desktop/mobile smoke passed: Pinavia shell present, stale NexusAI-only strings absent, three `/start-pilot` links, all room links present, no body overflow, desktop table retained, mobile table hidden, and five room cards rendered.
- Readiness result refresh deployed: `/readiness` has been patched so result-band copy and CTAs are Pinavia-first, old personal-email/NexusAI-only copy is gone, the AI-Native band uses teal rather than purple, advisor fallback uses `hello@pinavia.io`, and the contradictory advisor panel is hidden when the server returns a valid lane. Figma V0.7 frame `Launch Cockpit / 07 Readiness Result Path / 1440` (`143:2`) records commit `d1ac9b2` live-smoked. Live desktop/mobile result-flow smoke passed: AI-Native result rendered, inherited signup link produced, `/start-pilot` CTA present, no stale personal-email/NexusAI-only copy, no contradictory advisor panel, and no body overflow.
- Diagnostic intake honesty pass deployed: `/diagnostic` no longer claims an unwired paid checkout. The primary action is `Start diagnostic intake` to `/start-pilot?intent=diagnostic`, the secondary action remains `/readiness`, diagnostic pricing is hidden until checkout/receipt flows exist, and `/start-pilot` acknowledges diagnostic intent in the hero. Figma V0.7 frame `Launch Cockpit / 08 Diagnostic Intake Path / 1440` (`145:2`) was added and screenshot-verified after fixing route-card overflow. Deployed through `9428db9`; live apex desktop/mobile smoke passed for `/diagnostic`, `/start-pilot?intent=diagnostic`, and `/api/health`.
...
- Deployed through `ba3e440`: the full eight-check platform smoke passed on `https://app.pinavia.io`; `/`, `/diagnostic`, and `/invite/accept?code=missing` render; anonymous `GET /api/admin/trial-invites` and `POST /api/trial-invites/redeem` return 401; `/meridian` redirects signed-out visitors to `/sign-in?redirect_url=%2Fmeridian`. `https://pinavia.io` is the public landing host and is intentionally not an API CORS origin.
...
- Clerk custom domains use two hosts in production: `clerk.pinavia.io` serves the browser SDK and `accounts.pinavia.io` hosts sign-in/sign-up. `security-headers.ts` now derives and allowlists both from the frontend-domain and hosted-auth URL configuration. Keep `NEXT_PUBLIC_CLERK_DOMAIN` set to the frontend API host, not the hosted-auth host.
...
- Live `b268a25` deploy was confirmed, but auth smoke found missing hosted Clerk env values. Set the Render web-service values to `https://accounts.pinavia.io/sign-in` and `/sign-up`; the environment rebuild went live.
- Second live check found Clerk 404 because Nexus supplied a relative `redirect_url`, which Clerk resolved on `accounts.pinavia.io`. Sign-in/sign-up now build a same-origin absolute return URL from forwarded request host/protocol, falling back to `NEXT_PUBLIC_APP_URL`.
...
- **Session #58 delivered (2026-07-09) — Unknowns-first audit #3 + funnel gate + runbook v2:**
  - **Audit finding (map lag):** the 2026-07-08/09 sessions shipped the reviewer seat (incl. accepted-seat scorer gate + approval restricted to the bound reviewer), funnel, afterlife, and waitlist — but the demo runbook, TASKS calendar, and USER_STRATEGY still described the 2026-07-07 state. The runbook's seeding recipe ("name a reviewer") no longer cleared the gate.
  - **Territory finding (authz):** `requireScope` gives Clerk session users wildcard scope, so `read:admin`/`admin` scopes only constrain bearer tokens — the product had NO human operator concept. Any customer could open `/funnel` and read global acquisition counts.
  - **Funnel visibility policy (decision: admin-only now, flexible):** `NEXUS_FUNNEL_VISIBILITY` env (`admin` default / `workspace`), operator allowlist `NEXUS_OPERATOR_USER_IDS` (Clerk user ids), acquisition section operator-only in BOTH modes, 403 `funnel_operator_only`, nav entry opt-in via `NEXT_PUBLIC_NEXUS_FUNNEL_NAV` (hidden default). Pure policy helpers in `lib/services/funnel.ts` (`resolveFunnelVisibility`, `isOperatorUser`, `resolveFunnelAccess`) + tests in `tests/funnel.test.ts`. Envs added to `render.yaml`.
  - **Runbook v2:** `docs/DEMO_RUNBOOK_REGULATED.md` rewritten for the identity-bound reviewer — seeding order requires an ACCEPTED seat + approval from the reviewer account; §2a defines Mode A (live two-account loop, separate browser profiles, org-switch gotcha noted) vs Mode B (pre-staged seat) with a rehearsal go/no-go; landmine 4.4 flipped from "don't imply the seat exists" to "sell it"; migrations extended to 0033-0037; day-of checklist updated.
  - **Decisions (Ali, 2026-07-09):** demo = live two-account reviewer loop; funnel = admin-only everything for now (flexible via env); Pro intent goes public via the readiness flow post-demo. Recorded in `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-09; TASKS calendar plan updated to reflect shipped items 6-8.
  - **Verification caveat:** the sandbox mount degraded mid-session ("Resource deadlock avoided"); the new funnel tests and a full `vitest`/`tsc`/`npm run build` run MUST be executed on Ali's machine before pushing. Files changed: `lib/services/funnel.ts`, `app/api/funnel/route.ts`, `app/funnel/page.tsx`, `components/side-nav.tsx`, `tests/funnel.test.ts`, `render.yaml`, runbook + strategy + TASKS docs.
  - **Next slice:** push main + migrations 0033-0037 + envs + re-smoke (Ali); rehearse reviewer Mode A; then public Pro-interest capture on the readiness result.
...
- **Session #54 delivered (2026-07-07) — Demo week package (unknowns-first session #2):**
  - **Calendar set by Ali:** regulated-buyer demo ~week of 2026-07-13, public self-serve launch ~2026-08-04 (free + Pro waitlist, no checkout), paid pilot signing ~2026-08-18 (reviewer becomes a real Clerk-org seat before then; build must start by 2026-07-21). Stripe follows launch. Canonical: `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-07; execution: `TASKS.md` § Demo/Launch/Pilot Calendar Plan.
  - **Scorer signal confidence (cold-start honesty):** `computeSignalStrength` in `lib/services/workflow-twins.ts` — none/weak/moderate/strong from evidence + open decisions/actions; `payload.signal` on scorer runs; "Provisional" appended to weak/none summaries; informational `signal_strength` entry persisted inside pilotGates JSON (blocked: false, never gates, no migration); provisional line on `/workflows` recommendation card and Mission Control PilotStatusCard. Label informs, never blocks — human still confirms. Spec: `docs/WORKFLOW_TWIN_SCORER.md` §Signal confidence.
  - **Demo runbook:** new `docs/DEMO_RUNBOOK_REGULATED.md` — cutover preconditions (release gate), two-workspace demo model (live funnel workspace shows the honesty layer; seeded workspace shows full strength), walk order, landmines (subdomain fallbacks, 72h claim TTL, no-DB divergence, reviewer honesty, no ROI claims), day-of checklist.
  - **Regulated release-gate verification (same day follow-up):** active pilot host `https://app.pinavia.io` returned healthy `/api/health`; live `sentry-release` matched local `HEAD`/`origin/main` at `53b4d0ac76c2a729f451896b03602270f223260c`; authenticated browser checks proved readiness -> claim -> onboarding inheritance and rendered `/knowledge`, `/settings/connectors`, `/workflows`, and `/dashboard/ceo` live. Behavioral evidence strongly suggests migrations `0033` and `0034` are already applied in production.
  - **Release blocker found and fixed locally:** live `scripts/smoke-domain.mjs` still failed one check (`CORS allows expected origin`) because protected API `OPTIONS` requests were being intercepted by Clerk before middleware could emit the preflight response. Fixed in `middleware.ts` by returning API preflight responses before Clerk auth; regression added in `tests/security-headers.test.ts`. Verification after the fix: focused regression 36/36, full suite 62 files / 438 tests, `npm run build` exit 0. The fix still needs commit/deploy/re-smoke.
  - **Domain note:** `app.pinavia.io` and `nexus.pinavia.io` did not resolve in this shell during the release-gate run. Record as DNS/provider propagation pending, not app-code failure.
  - **Release-gate continuation (2026-07-08):** commit `920f562` was pushed and the active host served that exact SHA after cache-busting. `smoke-domain` passed on both `https://app.pinavia.io` and `https://app.pinavia.io`, and both `/api/health` endpoints were green.
  - **Fresh-workspace gate proof (2026-07-08):** on org/workspace `Nexus Gate Smoke 2026-07-08`, server-side `PATCH /api/strategy-profile` rejected a first workflow selection with `400 pilot_gates_unmet` and all three blocked gates (sponsor, reviewer, evidence). After enabling demo mode, `POST /api/workspace/demo-reset?sector=financial_services` seeded 5 evidence items; naming sponsor/reviewer and re-running the scorer flipped the fresh workspace to `pilotReady=true`; selecting `Decision & Action Twin` then succeeded and persisted.
  - **Operational nuance:** after switching Clerk organizations inside the browser, backend API routes did not always honor the new org until the session was refreshed. In browser automation the reliable sequence was `window.Clerk.setActive(...)` followed by `window.Clerk.session.touch()`. This is a same-session org-switch gotcha, not a blocker for demos that stay inside one chosen workspace.
  - **Named open gaps (doc-only, now backlogged):** pilot afterlife surface (post-selectedWorkflow loop + shadow ROI capture), funnel aggregation panel, low-confidence lane follow-up, Pro waitlist.
  - **Verification:** `tests/workflow-twins.test.ts` 9/9 (new signal test: thresholds, payload purity, persisted entry never gates); full suite 434/434 across 61 files. Sandbox artifact: `tests/google-oauth-config.test.ts` fails to LOAD (fs error -35, pre-existing mount issue, zero test failures) — re-verify locally. `implementation-notes.md` (top section) logs assumptions/deviations.
  - **Next slice:** execute the release gate + seed the demo workspace (Ali, external), then start the reviewer-seat implementation plan.
...
- **Write-side auth sweep (2026-07-07):** all mutating by-id routes confirmed to enforce workspace ownership (route-level checks or repository `WHERE id AND workspaceId`). One integrity gap fixed: `POST /api/actions` now validates the body `decisionId` is in-workspace (`decision_not_found`). Not a tenant breach. `docs/SECURITY_REVIEW.md` §1.2; regression `tests/actions-decision-fk.test.ts`.
- **Release gate runbook (2026-07-07):** `docs/RELEASE_GATE_2026-07-07.md` is the executable cutover for this session's work — preflight, apply migrations `0033`+`0034` (idempotent runner), required env vars (`NEXT_PUBLIC_APP_URL` critical; `NEXUS_RESEND_API_KEY` optional), deploy-SHA confirm, the authenticated six-step smoke (readiness→claim→inherit→scorer gates→confirm→server enforcement), and rollback. Run this before the next pilot demo. Local preflight passed here: 61 files / 430 tests, tsc clean, and `next build` clean.
...
- **Session #53 delivered (2026-07-06) — Readiness-to-onboarding lane inheritance (unknowns-first session):**
  ...
  - **Server-side reclassification hardening (same session, follow-up slice):** PATCH now enforces the lane-change rules itself — reason required (min 8 chars, `lane_change_reason_required`), regulated exit requires `regulatedExitConfirmed: true` (`regulated_exit_confirmation_required`), server stamps `laneChangedBy`/`laneChangedAt` defaults; `strategy_profile_updated` audit carries oldBuyerLane/newBuyerLane/reason/actor/regulated-exit flag. Spec §5.1. Tests: authz suite extended to 7 cases + `tests/strategy-profile-repository.test.ts`.
  - **Full API auth-bypass sweep (same session):** classified all 135 `app/api/` handlers; found + fixed 5 cross-tenant `workspaceId`-override routes (recommendations, pilot/paperwork [+added missing `read:admin` gate], settings/workspace [read+write of security settings + `read:settings`/`write:settings` enforcement], ingestion/status [bearer], strategy-profile [earlier]). Regression `tests/api-workspace-authz.test.ts` (6). Findings table in `docs/SECURITY_REVIEW.md` §1.1; BACKLOG "Manual API auth-bypass review" flipped to done; corrected the overstated "all routes use requireScope" claim. Verified with focused authz test, full suite 57 files / 415 tests, and production build; standalone `tsc` was inconclusive in this PTY.
  - **Workflow scorer → pilot bridge (same session, hardened 2026-07-07):** scorer now reads the strategy profile and applies buyer-lane fit boost; two-tier gating (hard gates = workflows unsuitable as first pilot, never recommended; bridgeable gates = sponsor/reviewer/evidence via `pilotGates` + `pilotReady`). `/workflows` card shows the gate checklist + "Confirm as first pilot" (writes `selectedWorkflow`). Server enforces `pilot_gates_unmet` on `PATCH /api/strategy-profile` using the scorer-owned `pilotReady` snapshot, so sponsor, reviewer, and evidence gates are all server-side. Fixed an unterminated-comment build break in `workflow-twins.ts` left by a prior half-edit. Doc reconciled: `docs/WORKFLOW_TWIN_SCORER.md` §Implemented Engine. Verified with focused scorer/profile tests, full suite 57 files / 420 tests, and production build. Standalone `tsc` was inconclusive in this PTY.
  - **Closed gap:** strategy profiles now have an in-memory fallback for no-DB demos. DB-backed environments still need migrations `0033` and `0034` before deploy so lane lifecycle and scorer readiness snapshots exist.
  ...
  - **Commit status:** ready to commit/push after local verification. Migrations 0033 and 0034 must run before deploy.
  - **Next slice:** Mission Control lane/strategy card; pruning job for expired submissions; claim-link product email via Resend; authenticated Render smoke after migrations/deploy.
...
- **Superseded:** `NEXT_PUBLIC_CLERK_DOMAIN` must be `clerk.pinavia.io` (the browser SDK host), not `accounts.pinavia.io` (the hosted-auth host). The 2026-07-25 two-host CSP correction derives and allowlists both hosts.
...
- **Verification next:** after production promotion, run `NODE_ENV=production node scripts/check-env.mjs` in the deploy environment and browser-smoke the hosted sign-in handoff through `https://accounts.pinavia.io/sign-in`.
...
## 2026-08-05 — Evidence classification cache and enforced evidence ceiling

- **Status:** `locally verified` and `committed but unpushed`. Local HEAD `94bb983`; pushed SHA still `ca0d964`, four commits behind (`48b07f2`, `8b87a4a`, `26f77f1`, `94bb983`). Deployed SHA not confirmed.
- **Slice ledger:** `docs/agent-runs/2026-08-05/evidence-classification-cache-and-ceiling-claude.md`. That ledger was opened retrospectively, after the commits landed, contrary to `nexus-papertrail`; its checkpoints are reconstructed from commit messages and captured verification output, and it says so.
- **Live check before any code.** `pinavia.io` serves normally and `/api/health` is ok across database, vector search, R2 originals, and the DeepSeek route; `/dashboard/ceo` renders live briefs and a real coverage map. A report that the live page was broken did not reproduce. Two things can read as broken: `/meridian` shows its correct cold-start panel, which on an unpopulated workspace is indistinguishable from a dead page and deserves a design pass; and `/evidence/review` 404s only because its commits were never pushed.
- **Defect found and fixed:** `/evidence/review` had no navigation entry, so it would have stayed unreachable even once deployed. Linked under Connect as "Untyped Evidence"; `/review` already owns the name "Review Queue".
- **Classification is now cached at ingest** (migration 0044, columns on `evidence_records`). Measured over 2,000 documents of ~50kB: read resolution 1,876ms to 2ms, the four native engines 8,978ms to 10ms, ingest 0.93ms/doc paid once. The cache carries `CLASSIFIER_VERSION`, a fingerprint derived from the classifier's own pattern table and thresholds rather than a hand-maintained constant, because a forgotten manual bump would leave workspaces answering coverage from retired rules with no visible symptom. Any row whose version does not match the running code is ignored and reclassified in place, so correctness never depends on the backfill; `POST /api/evidence/reclassify` only restores the speed.
- **The evidence ceiling sold on `/pricing` is now enforced.** `checkEvidenceLimit` had been correct and called from nowhere. It runs at `ingestEvidence`, the one chokepoint all thirteen ingest paths reach, for the same reason `matchesEvidenceTags` exists. It refuses rather than truncating and refuses before the record is built; it fails open, because a billing outage halting a paying pilot is worse than a reconcilable overage. The upload route re-checks before the R2 write so a refusal leaves no orphaned object, and answers 402 with counts and the next plan up.
- **Verification:** tsc 0; 1031 tests across 128 files; build boundaries clean; 197-page production build. Migration 0044 executed against real Postgres via PGlite (41/44, the 3 skips pgvector-only) with 11 assertions including idempotent re-run, the 2147483647 fingerprint bound, and legacy rows reading as no-cache. Negative controls run per guard: removing the version comparison failed 3 tests, disabling the ceiling failed 5; both reverted.
- **Not done, and not to be represented as done.** Seat enforcement: `maxTeam` cannot use this chokepoint because membership is granted through Clerk, so it needs a webhook on `organizationMembership.created`. Counting is already correct in the plan summary.
- **Pre-existing inconsistency, not introduced here:** `matchesEvidenceTags` ignores reviewer overrides, so the four native engines disagree with the Meridian coverage API about any document a human has retyped.
- **External blockers:** push requires credentials this environment does not hold. Migrations 0043 and 0044 are `migration pending` against production. Stripe remains paused pending real credentials.
- **Next exact action:** `git push`; then `npm run db:migrate && npm run db:check`; then re-smoke `/evidence/review` and `/meridian` against the newly deployed SHA before advancing status past `pushed / CI pending`.
