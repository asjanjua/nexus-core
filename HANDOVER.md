...
- Deployed through `ba3e440`: the full eight-check platform smoke passed on `https://app.pinavia.co`; `/`, `/diagnostic`, and `/invite/accept?code=missing` render; anonymous `GET /api/admin/trial-invites` and `POST /api/trial-invites/redeem` return 401; `/meridian` redirects signed-out visitors to `/sign-in?redirect_url=%2Fmeridian`. `https://pinavia.io` is the public landing host and is intentionally not an API CORS origin.
...
- **Session #54 delivered (2026-07-07) — Demo week package (unknowns-first session #2):**
  ...
  - **Release-gate continuation (2026-07-08):** commit `920f562` was pushed and the active host served that exact SHA after cache-busting. `smoke-domain` passed on both `https://app.pinavia.co` and `https://app.pinavia.io`, and both `/api/health` endpoints were green.
