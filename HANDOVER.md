...
- Product brief refresh in progress: `/product-brief` has been rewritten from old NexusAI-only collateral into a Pinavia family pilot brief with current room routes, buyer/workflow/proof/boundary rows, mobile product-room cards, pilot inputs/outputs, trust claims, and Start pilot/diagnostic follow-up. Figma V0.7 frame `Launch Cockpit / 06 Product Brief / 1440` (`141:2`) was added after fixing an initial auto-layout collapse. Local production build and desktop/mobile browser smoke passed; pending: push, Render deploy, and live `/product-brief` smoke.
