...
| Item | Status | Source | Notes |
|---|---|---|---|
| Approval policies (multi-level, teams) | open (P2) | `docs/APPROVAL_POLICIES_SPEC.md`, reviewer-seat slice 3 follow-on | One-line spec: layer a per-workspace approval policy (single reviewer today; later N-of-M, sequential chain, role/team-scoped, and delegate-on-absence) on top of the accepted `reviewer_seats` model, resolving the eligible approver set at approval time instead of the current single bound-reviewer check. Do not design the chain now — let the first pilot's real org chart define levels/roles so `reviewer_seats` extends cleanly (add a policy table + seat role/level, no rebuild). Break-glass bearer/admin override stays. |
