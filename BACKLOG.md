...
| Initial launch cockpit | done (Figma) | `docs/UI_V0_7_INITIAL_LAUNCH_COCKPIT.md`, Figma page `129:6`, frames `135:2` and `141:2` | Six screenshot-verified desktop frames consolidate final design sources, live demo routes, DNS blockers, acceptance checks, next actions, the `/start-pilot` conversion route, and the refreshed `/product-brief` collateral route for colleague review before pilots. |
...
| Product-family brief refresh | local verified, deployment pending | `/product-brief`, V0.7 frame `141:2` | Stale NexusAI-only product brief replaced with a Pinavia family pilot brief: current room routes, buyer/workflow/proof/boundary table on desktop, readable room cards on mobile, pilot inputs/outputs, trust claims, standing authority boundary, and Start pilot/diagnostic follow-up. Local production build and browser smoke passed; pending push, Render deploy, and live smoke. |
