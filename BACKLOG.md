...
| Nucleus methodology-pack authoring tool | partial, code-backed entry | `lib/nucleus-engagement-workflow.ts`, `lib/branding/white-label.ts`, `/nucleus`, `/nucleus/profile` | Nucleus now has its own engagement lifecycle registry, protected hub, and first writable profile screen. Firm name, logo URL, accent, and typeface persist through the guarded settings API; provenance, status meaning, approval boundaries, audit labels, and consequence previews remain fixed. Method-pack schema/versioning, client portal, and delivery controls remain planned. |
