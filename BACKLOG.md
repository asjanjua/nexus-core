...
| Richer Knowledge graph filters | in progress | `TASKS.md`, `docs/KNOWLEDGE_WORKSPACE.md` | Filter by tag, ref type, entity, source, freshness, and workflow. Code complete (`lib/knowledge/markdown.ts`, `lib/data/repository.ts`, `lib/data/store.ts`, `lib/knowledge/filter-params.ts`, both knowledge API routes, `knowledge-workspace.tsx` filter bar). Scoped `tsc --noEmit` passes clean. `npm test`/`npm run build` not run in sandbox — node_modules is synced from Ali's Mac (darwin-arm64 native bindings); sandbox is linux-arm64, so vitest/rolldown fails to load. Ali to run the 4-gate cycle on his machine before marking `done`. |
