...
| Meridian regulatory workflow | partial | `docs/MERIDIAN_REGULATORY_WORKFLOW.md`, `lib/meridian-regulatory-workflow.ts`, Figma nodes `87:3`, `160:2`, `/meridian/scope`, `/meridian/license-profile` | Meridian owns its domain lifecycle: scope, evidence, gap, and filing. The first runtime slice now persists the shared scope/profile record and cleanly gates the profile screen until Scope is saved. Requirement evidence, gap triage, and filing-pack routes remain planned; regulatory content still needs domain review. |
| Vantage DD Copilot workflow | partial, deployed | `docs/VANTAGE_DD_WORKFLOW.md`, `lib/vantage-dd-workflow.ts`, `/vantage`, `/vantage/coverage`, `f357ace` | Vantage has a protected launch hub plus an executable Coverage route. The route runs the first-party diligence skill against processed governed evidence, shows coverage, evidence gaps, critical gaps, passport exclusions, citations, and priority requests, and keeps the investment-decision boundary explicit. Live signed-out protection and health smoke passed; signed-in evidence/run/result smoke remains. Dealroom persistence plus red-flag, judgment, memo, and handoff routes remain planned. |
...
| Nucleus methodology-pack authoring tool | partial, deployed entry | `5e8e7d1`, `lib/nucleus-engagement-workflow.ts`, `lib/branding/white-label.ts`, `/nucleus`, `/nucleus/profile` | Nucleus now has its own engagement lifecycle registry, protected hub, and first writable profile screen. Firm name, logo URL, accent, and typeface persist through the guarded settings API; provenance, status meaning, approval boundaries, audit labels, and consequence previews remain fixed. Live signed-out protection and health smoke pass; signed-in save/reload, method-pack schema/versioning, client portal, and delivery controls remain planned. |
