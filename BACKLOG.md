...
| Vantage DD Copilot workflow | partial | `docs/VANTAGE_DD_WORKFLOW.md`, `lib/vantage-dd-workflow.ts`, `/vantage`, `/vantage/coverage` | Vantage has a protected launch hub plus an executable Coverage route. The route runs the first-party diligence skill against processed governed evidence, shows coverage, evidence gaps, critical gaps, passport exclusions, citations, and priority requests, and keeps the investment-decision boundary explicit. Dealroom persistence plus red-flag, judgment, memo, and handoff routes remain planned. |
