...
| Executable `knowledge_workspace_synthesis` native skill | open | `apps/mission-control/lib/agents/nexus-native-skills.ts`, `apps/mission-control/lib/services/knowledge.ts` | Last `planned` native skill. Synthesize governed notes, graph references, and evidence links into a source-backed workspace brief with a linked-evidence map and follow-up questions. Not approval-gated (analyze family). |
