...
| Pivot-suite runtimes — meridian + knowledge | open | `apps/mission-control/lib/agents/nexus-native-skills.ts`, `apps/mission-control/lib/domain/regulatory-requirement-library.ts` | Make `meridian_compliance_review` (reuse the regulatory requirement library) and `knowledge_workspace_synthesis` executable so every pivot suite has a real native runtime, reusing the checklist/citation pattern. |
