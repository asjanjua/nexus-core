...
7. **Protect the repository and release path.** Every code slice passes staged-tree preflight, build-boundary checks, timed local verification, and clean GitHub CI before Render deployment.
8. **Use one cross-agent delivery system.** Canonical workflows live under `.agents/skills/`; every meaningful slice gets a unique `docs/agent-runs/` ledger, parallel writers use disjoint worktrees, and only the integration agent reconciles central operating documents.
9. **Keep the runtime on a supported LTS line.** Node 24 is the production baseline; Node 22.12+ remains a temporary compatibility rung. CI proves both while Render and local defaults stay on Node 24.

## Architecture Watch: API Extraction

Status: `watch`, not an active build. Keep the pilot as a modular monolith. Reassess a separate API/worker service only when measured long-running jobs, independent scaling/release cadence, compliance isolation, or external API volume meets a trigger in `docs/API_SERVICE_BOUNDARY_DECISION.md`.
...
| Initial launch cockpit | done (Figma) | `docs/UI_V0_7_INITIAL_LAUNCH_COCKPIT.md`, Figma page `129:6`, frames `135:2`, `141:2`, `143:2`, `145:2`, `160:2`, `162:2`, `164:2`, `167:2`, `167:57`, `173:2`, `176:2`, `176:63`, `182:2`, `182:74`, `182:146`, `182:218`, `182:290`, `182:362` | Twenty-two screenshot-verified desktop frames consolidate final design sources, live demo routes, public conversion, connector intake, and full pilot handoff stories. The six end-to-end handoffs are design-final planned deep routes, not deployed capabilities. |
| Initial launch UI release audit | done | `docs/INITIAL_LAUNCH_UI_RELEASE_AUDIT.md` | Per-surface evidence matrix distinguishes design-final, code-backed, live-proven, and authenticated proof still required. This is the release reference for a controlled pilot demonstration. |
| Meridian launch hub polish | done (code) | `components/meridian-submission-panel.tsx`, `lib/meridian-regulatory-workflow.ts` | Meridian now matches the Vantage/Nucleus launch-hub pattern: planned screen cards expose user inputs and action points, planned deep-route status is explicit, and global jurisdiction-pack requirements are visible before customer/regulator claims. |
| Pilot-start intake page | done | `/start-pilot`, V0.7 frame `135:2`, `1b8c5f4` | Redirect-only handoff replaced with a public Pinavia pilot-start page: required kickoff inputs, single primary workspace CTA, diagnostic/email secondary CTAs, product-room preview routes, and explicit human-authority boundary. Live apex smoke passed: `/start-pilot` returns 200 and the page contains the Pinavia shell, headline, and workspace CTA. |
| Landing CTA alignment to pilot start | done | `/`, public shell, V0.7 frame `135:2`, `90b911a` | Homepage hero, public header, money-map CTA, competitive-difference CTA, footer, and final pilot CTA now converge on `/start-pilot`; readiness remains the diagnostic secondary path. Product-family cards now link Meridian, Vantage, and Nucleus to their live protected hubs. Live apex smoke passed: no `Design candidate` copy remains, six homepage links point to `/start-pilot`, and repeated Meridian/Vantage/Nucleus links point to their live hubs. |
...
| Define Render cron jobs | code-complete, activation pending | `render.cron.yaml`, `TASKS.md`, architecture review | Dispatch, billing, synthesis, and readiness-prune are defined as opt-in paid `starter` services in `render.cron.yaml`. All five API routes built (`/api/cron/*`) with `cronAuthorized()` guards. Remaining ops: set `NEXUS_CRON_SECRET` in Render, attach `render.cron.yaml` Blueprint, approve billing. |
...
| Manual API auth-bypass review | done | `docs/SECURITY_REVIEW.md` §1.1–1.2 | Read-side sweep 2026-07-06: 5 cross-tenant `workspaceId`-override routes fixed. Write-side sweep 2026-07-07: all by-id mutations confirmed to enforce ownership; one integrity gap fixed (`POST /api/actions` unvalidated `decisionId`). Regressions `tests/api-workspace-authz.test.ts`, `tests/actions-decision-fk.test.ts` + strategy-profile suite. Residual: no cross-tenant read or write path remains by design. |
...
| Uptime monitoring | code-complete, external service pending | `TASKS.md`, `/api/health` | Health endpoint checks database, LLM, vector search, originals storage, and build info. Returns `status: ok` or `degraded`. Point any external monitor (Better Uptime, Pingdom, UptimeRobot) at `https://app.pinavia.io/api/health`. |
| Automated dependency scanning in CI/deploy | done; one reviewed high residual | `.github/workflows/ci.yml`, `CODE_REVIEW_2026-07-25.md` | CI hard-fails on critical advisories and reports high/moderate findings. As of 2026-07-25, the Sharp/libvips high is resolved by the `next.sharp` 0.35.3 override; the sole remaining high is Next 15's exactly pinned PostCSS build-time dependency. Do not suppress it: resolve through a tested Next major upgrade. |
| Neon daily backups and restore test | shipped 2026-08-06 | `TASKS.md`, `docs/PRODUCTION_HEALTH_CHECKLIST.md` | 30-day retention target. |
| R2 bucket versioning if originals storage is promised | shipped 2026-08-06 | `TASKS.md`, `docs/PRODUCTION_HEALTH_CHECKLIST.md` | Required for original-file recovery commitments. |
| Support/security mailbox monitored | in progress (contacts documented, DNS pending) | `TASKS.md`, `docs/SECURITY_REVIEW.md` | `support@...` and `security@...` or equivalent before paid pilot. |
| Pilot SLA documented and linked in paperwork | shipped 2026-08-06 | `TASKS.md` | 4-hour response, 1-hour critical target in current task text. |
| Readiness-to-onboarding lane inheritance | live verified, release gate open | `docs/LANE_ASSIGNMENT_SPEC.md`, `CHANGELOG.md`, `docs/RELEASE_GATE_2026-07-07.md` | Shipped 2026-07-06 (session #53): server-side lane assignment, single-use claim codes (migration 0033), wizard inheritance + server-enforced reclassification checkpoint, enriched lane-change audit payloads, strategy-profile authz fix. Hardened 2026-07-07: claim-link product email via Resend boundary, protected readiness-prune cron, and Mission Control pilot-status card. Live 2026-07-07 evidence: public readiness -> claim URL -> onboarding inheritance -> authenticated dashboard all verified on the active pilot host. Remaining gate: deploy the API preflight/CORS middleware fix and re-run `smoke:domain`; `.io` product domains remain DNS-pending here. |
| Architecture-review adoption sequence (2026-07-10) | in progress | `docs/ARCH_REVIEW_2026-07-10_ADOPTION.md` | Second repo verification complete: eval was the sole application LLM caller missing policy routing and now uses `audit_refusal`; existing build-boundary checks are narrower than full dependency-direction enforcement. Pre-demo external controls remain: non-sleeping instances, Neon branch protection/snapshots/restore window, uptime monitoring. Demo->launch: dedicated Render worker + job lease/dead-letter/idempotency/trace, ingestion malware/MIME hardening, minimal OTEL, restore drill. Launch->pilot: DB roles, RLS defense-in-depth, `operator_assignments` table, audit hash chaining, credential key separation, email deliverability. |
| Reviewer as a real second seat (Clerk org + reviewer role + invite + identity-bound approvals) | local hardening complete, deployment and behavior smoke pending | `docs/DEMO_ACCESS_RUNBOOK.md` | Migration 0035 and the authenticated management page are live; acceptance hardening locally requires the invited verified Clerk email and a non-admin organization member, so an administrator cannot self-accept. Remaining production gate: deploy, then two-account invite, acceptance, scorer, and bound-reviewer approval smoke. General workspace member roles remain a separate Phase 9 item. |
| Scorer signal-confidence labels (cold-start honesty) | local verified | `docs/WORKFLOW_TWIN_SCORER.md` §Signal confidence, `CHANGELOG.md` | Shipped 2026-07-07: `computeSignalStrength` in scorer runs, provisional copy on `/workflows` and pilot-status card, informational `signal_strength` entry in persisted pilotGates (blocked: false, never gates). Tests in `tests/workflow-twins.test.ts`. |
| Workflow scorer as governed pilot bridge | live verified, release gate open | `docs/WORKFLOW_TWIN_SCORER.md`, `CHANGELOG.md`, `docs/RELEASE_GATE_2026-07-07.md` | Shipped 2026-07-06 (session #53), hardened 2026-07-07: scorer reads strategy profile, lane-fit boost, two-tier gating (hard gates never recommended; bridgeable sponsor/reviewer/evidence gates persisted as `pilotReady`/`pilotGates`), `/workflows` confirm-first-pilot action, server-enforced `pilot_gates_unmet` on selectedWorkflow, no-DB strategy-profile fallback for demos, and Mission Control pilot-status visibility. Live 2026-07-07 evidence: authenticated `/workflows` and `/dashboard/ceo` rendered the selected workflow plus ready sponsor/reviewer/evidence gates on the active pilot host. Remaining gate: deploy the protected-route API preflight fix, then re-smoke the active host and re-check the blocked-workspace `pilot_gates_unmet` trace. |
...
| Item | Status | Source | Notes |
|---|---|---|---|
| Approval policies (multi-level, teams) | resolver + API + Settings UI shipped 2026-08-06 | `docs/APPROVAL_POLICIES_SPEC.md`, reviewer-seat slice 3 follow-on | Migration 0046 shipped: extended `reviewer_seats` with role/level/team, added `approval_policies` table. `resolveApprovalDecision` resolver covers single/n_of_m/sequential/role_scoped modes. Wired into POST /api/approvals. GET/PUT /api/approval-policy routes built. Policy-aware buildPilotGates staffing check. 24 test assertions. Settings UI for policy editing remains per spec §7. |
| Funnel + pilot-lifecycle operator panel | local verified, deployment pending | `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-09 | `/funnel` and `/api/funnel` aggregate readiness and workspace lifecycle signals. Access is now policy-driven and operator-only by default; cross-tenant acquisition remains operator-only in every mode. Remaining: deploy env allowlist and authenticated operator smoke. |
| Pro waitlist for public launch (no checkout) | deployed, behavior smoke pending | `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-09 | Migration 0037 is included in the live ordered migration run; authenticated waitlist API/page and email paths are built. Remaining: behavior smoke plus the post-demo public-readiness handoff into the authenticated waitlist. |
| Pilot afterlife surface (run loop, shadow ROI capture, expand/stop record) | deployed, behavior smoke pending | `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-07, `docs/SHADOW_MODE_ROI_PLAYBOOK.md` | Migration 0036 is included in the live ordered migration run; `/pilot/afterlife`, shadow-ROI capture, sponsor digest, and audited expand/hold/stop outcomes are built. Remaining: behavior smoke and propagation into pilot paperwork and the scorecard. |
...
| August–September 2026 pilot and Quorum decision capture | decision-ready, nominations pending | `docs/AUGUST_SEPTEMBER_2026_MILESTONE_DECISION_PACKET.md` | Single source for the two required external nominations, controlled proof acceptance, and the evidence gate before any commercial SOW is treated as signable. It intentionally contains no invented partner, identity, pack, or authority. |
...
| Duplicate/contradiction/staleness audit | shipped 2026-08-06 | `lib/knowledge-audit.ts` | Structural audit engine with duplicate, contradiction, and staleness detection. API route at `/api/knowledge/audit`, UI panel at `/knowledge`. 10 tests. |
| Daily/project/workflow brief automation from notes | shipped 2026-08-06 | `lib/daily-brief.ts` | Daily brief generator: 24h activity, pending decisions, untagged notes, stale items. API at `/api/knowledge/daily-brief`, UI panel at `/knowledge`. |
...
| Ops Review Twin richer UI | shipped 2026-08-06 | `components/ops-review-panel.tsx` | Weekly execution summary, blockers list, overdue owners, KPI signal cards. Panel on `/workflows`. |
| Trusted eval scorecards from U3/U4 data | shipped 2026-08-06 | `components/eval-scorecard.tsx` | Pass rate, confidence, latency, category breakdown, trend strip, per-case keyword analysis. Page at `/eval`. |
...
| Executable `vantage_diligence_analysis` native skill | done | `apps/mission-control/lib/agents/vantage-diligence-analysis.ts`, `apps/mission-control/lib/services/vantage-diligence-analysis-runner.ts`, `apps/mission-control/app/api/agents/native-skills/vantage-diligence-analysis/route.ts`, `apps/mission-control/tests/vantage-diligence-analysis.test.ts` | First executable pivot-suite runtime. Composes the DD checklist library with the grid citation model: diligence coverage, red flags with indicators, model tie-outs, IC memo sections, and a coverage-signal recommendation. Endpoint + "Run diligence analysis" Settings button. Skill is `runtime_ready`. |
| Executable `quorum_governance_review` native skill | done | `apps/mission-control/lib/agents/quorum-governance-review.ts`, `apps/mission-control/lib/services/quorum-governance-review-runner.ts`, `apps/mission-control/app/api/agents/native-skills/quorum-governance-review/route.ts`, `apps/mission-control/tests/quorum-governance-review.test.ts` | Reviews board evidence + decisions + actions against a board-pack checklist and the Quorum governance boundaries: governance findings, decision gaps, approval packet, board-pack caveats. Endpoint + "Run governance review" Settings button. Skill is `runtime_ready`. |
| Executable `meridian_compliance_review` native skill | done | `apps/mission-control/lib/agents/meridian-compliance-review.ts`, `apps/mission-control/lib/services/meridian-compliance-review-runner.ts`, `apps/mission-control/app/api/agents/native-skills/meridian-compliance-review/route.ts`, `apps/mission-control/tests/meridian-compliance-review.test.ts` | Maps regulator license-type requirements to governed evidence: requirement coverage, compliance gaps with indicators, qualified-reviewer packet, filing caveats. Endpoint + "Run compliance review" Settings button. Skill is `runtime_ready`. |
| Executable `knowledge_workspace_synthesis` native skill | shipped 2026-08-06 | `apps/mission-control/lib/agents/nexus-native-skills.ts`, `apps/mission-control/lib/services/knowledge.ts` | Last `planned` native skill. Synthesize governed notes, graph references, and evidence links into a source-backed workspace brief with a linked-evidence map and follow-up questions. Not approval-gated (analyze family). |
...
| Meridian regulatory workflow | partial, code-backed handoff | `docs/MERIDIAN_REGULATORY_WORKFLOW.md`, `lib/meridian-regulatory-workflow.ts`, Figma nodes `87:3`, `160:2`, `182:2`, `182:74`, `/meridian/scope`, `/meridian/license-profile`, `/meridian/evidence-coverage`, `/meridian/filing-pack` | Meridian owns its domain lifecycle: scope, evidence, gap, and filing. Scope/profile persists the shared record. Evidence coverage and filing pack now provide protected interactive review workbenches that hand off named human decisions; requirement/evidence engines and regulatory content still need specialist review. |
| Vantage DD Copilot workflow | partial, deployed handoff | `docs/VANTAGE_DD_WORKFLOW.md`, `lib/vantage-dd-workflow.ts`, `/vantage`, `/vantage/coverage`, `/vantage/red-flags`, `f357ace` | Vantage has a protected launch hub, an executable Coverage route, and a protected red-flag/IC review workbench that drafts a human-owned decision handoff. It does not claim investment judgment or persist an IC conclusion. Live signed-out protection and health smoke passed for the earlier slice; signed-in evidence/run/result smoke remains. |
...
| Nucleus methodology-pack authoring tool | partial, deployed handoff | `5e8e7d1`, `lib/nucleus-engagement-workflow.ts`, `lib/branding/white-label.ts`, `/nucleus`, `/nucleus/profile`, `/nucleus/reviewer-console` | Nucleus now has its own engagement lifecycle registry, protected hub, first writable profile screen, and protected partner-review/client-preview workbench that drafts a human decision. Firm name, logo URL, accent, and typeface persist through the guarded settings API; provenance, status meaning, approval boundaries, audit labels, and consequence previews remain fixed. Signed-in save/reload and method-pack persistence remain due. |
...
| Connector scheduler and sync history | shipped 2026-08-06 | high | Needed for non-event-based source freshness and retry visibility. |
...
| Empty/partial/blocked states pass | shipped (verified — empty states present in approvals, knowledge, members, onboarding) | `docs/UI_UX_EXPERT_REVIEW_2026-06-16.md` | Make blocked connectors, insufficient evidence, denied outputs, and loading states first-class. |
