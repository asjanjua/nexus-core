...
| Initial launch cockpit | done (Figma) | `docs/UI_V0_7_INITIAL_LAUNCH_COCKPIT.md`, Figma page `129:6`, frames `135:2`, `141:2`, `143:2` | Seven screenshot-verified desktop frames consolidate final design sources, live demo routes, DNS blockers, acceptance checks, next actions, the `/start-pilot` conversion route, the refreshed `/product-brief` collateral route, and the `/readiness` result conversion path for colleague review before pilots. |
...
| Readiness result path refresh | local verified, deployment pending | `/readiness`, V0.7 frame `143:2` | Stale NexusAI-only result copy and personal-email CTAs replaced with Pinavia result copy, `/diagnostic` or `/start-pilot` band CTAs, `hello@pinavia.io` advisor fallback, teal top-band treatment, and a single result path when a lane exists. Local desktop/mobile result-flow smoke passed; pending push, Render deploy, and live smoke. |
