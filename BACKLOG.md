...
| Pivot-suite runtimes — quorum + meridian | open | `apps/mission-control/lib/agents/nexus-native-skills.ts`, `apps/mission-control/lib/domain/regulatory-requirement-library.ts` | Make `quorum_governance_review` and `meridian_compliance_review` executable so every pivot suite has a real native runtime, reusing the checklist/citation pattern. |
