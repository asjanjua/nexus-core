...
| Manual API auth-bypass review | done | `docs/SECURITY_REVIEW.md` §1.1 | Full sweep completed 2026-07-06: all 135 `app/api/` handlers classified. Found and fixed 5 cross-tenant `workspaceId`-override routes (strategy-profile, recommendations, pilot/paperwork [+missing scope gate], settings/workspace [read+write + settings scope enforcement], ingestion/status [bearer]). Regression `tests/api-workspace-authz.test.ts` + strategy-profile suite. Residual: no operator multi-tenant path remains by design. |
