...
| Initial launch cockpit | done (Figma) | `docs/UI_V0_7_INITIAL_LAUNCH_COCKPIT.md`, Figma page `129:6`, frames `135:2`, `141:2`, `143:2`, `145:2` | Eight screenshot-verified desktop frames consolidate final design sources, live demo routes, DNS blockers, acceptance checks, next actions, the `/start-pilot` conversion route, the refreshed `/product-brief` collateral route, the `/readiness` result conversion path, and the `/diagnostic` intake path for colleague review before pilots. |
...
| Diagnostic intake honesty pass | code-backed candidate | `/diagnostic`, `/start-pilot?intent=diagnostic`, V0.7 frame `145:2` | The diagnostic page no longer advertises an unwired paid checkout. Primary CTA now reads `Start diagnostic intake` and routes to `/start-pilot?intent=diagnostic`; `/start-pilot` acknowledges diagnostic intent; the page keeps `/readiness` as secondary, hides pricing while checkout/receipt flows are absent, and states the no-certification/no-filing/no-legal-opinion/no-approval boundary. Local TypeScript, production build, and desktop/mobile route smoke passed; deploy and live smoke pending. |
...
| Vantage DD Copilot workflow | partial | `docs/VANTAGE_DD_WORKFLOW.md`, `lib/vantage-dd-workflow.ts`, `/vantage` | Vantage now has a protected launch hub with the registry-driven dealroom/coverage/redflags/memo arcs, user input/action guidance, and no-investment-decision boundary. Deep `/vantage/*` routes and live deal workspace APIs are still planned. |
