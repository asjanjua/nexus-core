...
| Reviewer as a real second seat (Clerk org + reviewer role + invite + identity-bound approvals) | open | `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-07 | Decided 2026-07-07: reviewer must be a real actor before pilot signing (~6 weeks out). Today sponsor/reviewer are strings in a single-seat workspace; approvals are self-approval. Start within ~2 weeks. Until shipped, pilot paperwork must state the limitation. |
...
| Funnel + pilot-lifecycle operator panel | open | `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-07 | Aggregate the existing funnel audit events (`readiness.assessment_submitted`, `readiness.claim_redeemed`, scorer/confirm events) into an operator view plus a pilot-stage tracker (selected -> evidence -> first brief -> review loop -> ROI). Target: launch preparation (~4 weeks). Also a demo asset for regulated buyers. |
| Pro waitlist for public launch (no checkout) | open | `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-07 | Launch is free; show pricing, collect Pro intent via waitlist, invoice pilots. Stripe checkout follows post-launch. |
| Pilot afterlife surface (run loop, shadow ROI capture, expand/stop record) | open | `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-07, `docs/SHADOW_MODE_ROI_PLAYBOOK.md` | The funnel's last four stages are doc-only after `selectedWorkflow` is written. Build alongside the reviewer seat before pilot signing. |
