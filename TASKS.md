...
3. [ ] Rehearse reviewer demo Mode A (live two-account loop, separate browser profiles) per runbook §2a; lock Mode B fallback if the org switch misbehaves twice.
...
6. [x] Reviewer-seat build — DONE ahead of the 2026-07-21 hard date: migration 0035, invite flow + resend/expiry, accept binds Clerk identity, scorer gate requires accepted seat, approval rights restricted to bound reviewer (`403 approval_requires_bound_reviewer`), management page. Migration-backed management read path is live and clean as of 2026-07-15; the two-account invite/accept/approval rehearsal remains a separate demo gate.
