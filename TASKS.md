...
32. [ ] Deploy Commercial Pilot Loop V0.4 — the full eight-check platform smoke is complete on `app.pinavia.io`; `/`, `/diagnostic`, and `/invite/accept?code=missing` render; anonymous `GET /api/admin/trial-invites` and `POST /api/trial-invites/redeem` return 401; `/meridian` preserves the sign-in return path. `pinavia.io` is the public landing host and intentionally not an API CORS origin. Remaining: set `PINAVIA_ADMIN_PRINCIPALS`, then smoke authenticated `/admin/invites` and a real invite redemption with server-side sample seeding.
