...
**Commercial Pilot Loop V0.4 (2026-07-26):** Figma candidate created in `Nexus System` at `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=121-2`. It maps the current commercial tree across `/`, `/diagnostic`, `/meridian`, `/admin/invites`, and `/invite/accept`. The full platform smoke passes on `app.pinavia.io`; `pinavia.io` remains the public landing host. Authenticated staff smoke still requires `PINAVIA_ADMIN_PRINCIPALS`.

**Initial Launch Final Pack V0.5 (2026-07-26):** Figma launch-lock board created at `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=125-2`. It records the launch truth for all pilot surfaces. V0.6 now supersedes its Vantage/Nucleus status: protected launch hubs exist at `/vantage` and `/nucleus`; deeper route trees remain planned.

**Launch Route Update V0.6 (2026-07-26):** Figma page `23 Launch Route Update V0.6` created at `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=126-2`. Commit `20f2848` adds protected Vantage and Nucleus route entries, Nucleus's dedicated workflow registry, and product-domain sign-in redirects to those hubs. Local tests/typecheck/build passed; live signed-out smoke passed on `pinavia.io` and the Render service URL.

**Initial Launch Cockpit V0.7 (2026-07-26):** Figma page `24 Initial Launch Cockpit V0.7` created at `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=129-6`. It consolidates the colleague-review view across final design sources, live route truth, DNS blockers, acceptance checks, the next pilot action board, the `/start-pilot` intake route, the refreshed `/product-brief` collateral route, the `/readiness` result conversion path, and the `/diagnostic` intake path. Eight 1440x900 frames were screenshot-verified after layout fixes.

**Meridian Launch Hub Polish (2026-07-26):** `/meridian` now exposes user inputs, action points, planned deep-route labels, and global jurisdiction-pack requirements directly in the live Submission Room. This brings Meridian up to the same launch-hub clarity standard as Vantage and Nucleus while preserving the no-filing/no-certification boundary.

**Pilot Start Intake Page (2026-07-26):** `/start-pilot` is now a public Pinavia intake page instead of a redirect-only route. It states required kickoff inputs, keeps `Create pilot workspace` as the single primary action, preserves diagnostic/email as secondary CTAs, and shows product-room preview routes with human-authority boundaries. Deployed through `1b8c5f4`; live apex smoke returns 200 and page-content checks find the Pinavia shell, headline, and workspace CTA.

**Landing CTA Alignment (2026-07-26):** The public homepage and shell now treat `/start-pilot` as the primary conversion path. Hero, header, money-map, competitive-difference, footer, and final pilot CTAs converge on pilot start; `Run the diagnostic` remains secondary. Product-family cards point to live protected hubs for Meridian, Vantage, and Nucleus instead of describing them as design-only candidates. Deployed through `90b911a`; live apex smoke confirms the new CTA/link map.

**Product-Family Brief Refresh (2026-07-26):** `/product-brief` has been reworked from stale NexusAI-only collateral into a Pinavia product-family pilot brief. It now carries current live routes, buyer/workflow/proof/boundary rows for all five rooms, mobile product-room cards, pilot inputs/outputs, trust claims, the standing authority boundary, and Start pilot/diagnostic follow-up. Deployed through `4e8b71f`; live desktop/mobile smoke passed with no stale NexusAI-only strings, no mobile overflow, desktop table retained, mobile table hidden, and five room cards rendered.

**Readiness Result Path Refresh (2026-07-27):** `/readiness` result copy now uses Pinavia instead of stale NexusAI-only messaging, routes band CTAs to `/diagnostic` or `/start-pilot`, uses `hello@pinavia.io` only for advisor fallback, switches the top band from purple to teal, and suppresses the contradictory advisor card whenever a lane and inherited-result signup path exist. Deployed through `d1ac9b2`; live desktop/mobile result-flow smoke passed.

**Diagnostic Intake Honesty Pass (2026-07-27):** `/diagnostic` now avoids implying a live checkout for the diagnostic while the payment/receipt flow is not implemented. The primary CTA reads `Start diagnostic intake` and routes to `/start-pilot?intent=diagnostic`, the secondary path remains `/readiness`, and `/start-pilot` changes its hero copy when diagnostic intent is present. Figma V0.7 frame `Launch Cockpit / 08 Diagnostic Intake Path / 1440` (`145:2`) records the updated handoff and acceptance checks. Deployed through `9428db9`; live apex desktop/mobile smoke passed.
...
8. [x] Confirm Render deploy for the latest pushed application commit, then run authenticated smoke tests in the logged-in browser session. Closed 2026-07-15: Render served `32166903b55b2ce8239bd5eb21fc0bd4121811e2`; public smoke passed 8/8; dashboard, `/knowledge`, `/workflows`, `/settings/connectors`, and `/reviewer-seat` rendered authenticated with a clean Nexus console after the Clerk refresh window.
9. [x] Define cron jobs -- dispatch runner, billing reset, trial-to-free conversion, synthesis, and readiness submission pruning. The cost-bearing Render `starter` resources now live in opt-in `render.cron.yaml`, not the primary web-only Blueprint. Definitions are complete; activation, secret injection, and billing approval remain operational work.
...
28. [x] Add product subdomain detection for the Pinavia house-of-brands — `lib/product-detection.ts`, middleware `x-nexus-product`, product CORS origins, public/auth shell product lockup, and route-safe sign-in redirects. `quorum.pinavia.io` lands on `/board`; Meridian, Vantage, and Nucleus now land on `/meridian`, `/vantage`, and `/nucleus` once the V0.6 route-entry commit is deployed.
29. [x] Complete product subdomain entry layer — Cloudflare now provides query-preserving 301 redirects for `nexus`, `quorum`, `meridian`, `vantage`, and `nucleus.pinavia.io` into canonical `app.pinavia.io` routes. Separate Render custom domains and Clerk origins are intentionally not required. Verified `quorum` redirect again on 2026-07-29; canonical-app health remains a separate release gate in `docs/PRODUCTION_HEALTH_CHECKLIST.md`.
...
32. [ ] Complete Commercial Pilot Loop V0.4 authenticated proof — local Node 24 preflight passed on 2026-07-29 (729 tests, TypeScript, boundaries, production build); a manual latest-commit Render deploy was initiated and canonical `app.pinavia.io` passed all 8 public domain checks. Confirm the SHA in Render once its dashboard recovers, set `PINAVIA_ADMIN_PRINCIPALS`, then smoke authenticated `/admin/invites` and a real invite redemption with server-side sample seeding.
33. [x] Create Initial Launch Final Pack V0.5 — page `22 Initial Launch UI Final Pack V0.5` (`125:2`) adds the readiness matrix, core buyer path, pilot surface lock, handoff index, demo gaps, and ten-minute pilot demo script for all launch surfaces.
34. [x] Convert the V0.5 launch order into app changes — Vantage/Nucleus protected route entries are implemented in `20f2848`, mirrored in Figma V0.6, deployed, and live-smoked signed-out. Deep routes and staff invite smoke remain separate backlog items.
35. [x] Create Initial Launch Cockpit V0.7 — page `24 Initial Launch Cockpit V0.7` (`129:6`) adds the colleague-review cockpit over V0.4/V0.5/V0.6 with executive status, demo route map, final screen QA matrix, action board, pilot-start intake frame, product-brief refresh frame, and readiness-result path frame. Seven 1440x900 desktop frames were screenshot-verified after layout fixes.
36. [x] Polish Meridian live launch hub — `/meridian` now shows screen-specific user inputs/action points, planned deep-route labels, and global jurisdiction-pack requirements. Verified with focused Meridian registry tests, standalone TypeScript, and production build.
37. [x] Add public pilot-start intake page — `/start-pilot` now renders the kickoff input list, single primary workspace CTA, diagnostic/email secondary CTAs, product-room preview routes, and authority boundary. Local build/screenshot verification passed, commit `1b8c5f4` was pushed, Render deployed it, and live apex smoke returns 200 with the expected page content.
38. [x] Align public landing CTAs to pilot start — homepage hero, header, money-map CTA, competitive-difference CTA, footer, and final pilot CTA now converge on `/start-pilot`; product-family cards point to live protected hubs. Local build/browser verification passed, commit `90b911a` was pushed, Render deployed it, and live apex smoke confirms the new CTA/link map.
39. [x] Refresh product-family brief — `/product-brief` now presents Pinavia as the product family instead of a stale NexusAI-only one-pager, with live room routes, mobile room cards, and authority boundaries. Local production build passed, `4c17bb3` and `4e8b71f` were pushed, Render deployed, and live desktop/mobile smoke passed.
40. [x] Refresh readiness result path — `/readiness` now routes completed assessments into Pinavia pilot/diagnostic paths instead of old NexusAI mailto copy, and Figma frame `143:2` records the change. Local production build passed, `d1ac9b2` was pushed and deployed, and live desktop/mobile result-flow smoke passed.
41. [x] Refresh diagnostic intake path — `/diagnostic` no longer advertises an unwired paid checkout; it routes to `/start-pilot?intent=diagnostic`, keeps `/readiness` as secondary, hides diagnostic pricing until checkout/receipt flows exist, and documents the handoff in Figma frame `145:2`. Local TypeScript, production build, local route smoke, and live apex desktop/mobile smoke passed after deploy through `9428db9`.
41. [x] Repair `relay.py` as an append-only papertrail adapter — locked exact-prefix handover appends, unique dated ledgers, fingerprint duplicate protection, no-write preview modes, explicit-file commit safety, concurrent-writer serialization, and six root test regressions completed 2026-07-15.
...
34. [x] Build the Pinavia trial-invite portal locally — migration 0038, a fail-closed `PINAVIA_ADMIN_PRINCIPALS` platform-staff gate, single-use hashed invite links, Pro-plan trial provisioning, optional demo-pack seeding, Resend/manual delivery, `/admin/invites`, and `/invite/accept` shipped in local commit `bb2af6c`. Local verification reported 83 files / 576 tests, typecheck, boundaries, and production build clean.
35. [~] Production-enable the Pinavia trial-invite portal — production `main` is release-green and the canonical app passed public smoke. Neon `main` now confirms migration 0038 (`trial_invites` plus required schema columns), and Render has the staff-only `PINAVIA_ADMIN_PRINCIPALS` allowlist. Remaining: an authorised real invite → separate controlled identity sign-up → redeem → empty-workspace demo-pack smoke, audit-event check, and non-staff denial check. Never put customer-admin principals in this allowlist or disclose a bearer invite link.
36. [~] Record the August–September 2026 pilot and Quorum nominations — `docs/AUGUST_SEPTEMBER_2026_MILESTONE_DECISION_PACKET.md` provides the decision record, proof acceptance, and SOW release gate. Render staff allowlisting and exact deployment identity are live; production migration confirmation, a controlled second identity, and both external nominations remain pending. Do not use the packet to infer a partner, authority, or commercial commitment.

## Demo/Launch/Pilot Calendar Plan (added 2026-07-07, updated 2026-07-09)

Three dated events drive priority (decisions in `docs/USER_STRATEGY_AND_PIVOTS.md` §Decisions 2026-07-07 and §Decisions 2026-07-09): regulated-buyer demo ~week of 2026-07-13, public self-serve launch ~2026-08-04, paid pilot signing ~2026-08-18.

**Week 1 — demo (runbook: `docs/DEMO_RUNBOOK_REGULATED.md`, rewritten 2026-07-09 for the identity-bound reviewer gate):**
1. [~] Release gate — Node 24 application commit `3216690` was live on `app.pinavia.io` on 2026-07-15; CI/CodeQL passed, public smoke passed 8/8, hosted-Clerk session refresh held, and the migration-backed reviewer-seat read path rendered. Remaining external/product gates: confirm operator allowlist and optional ops/Resend envs, run scheduled email, and re-smoke any additional product domains used in a demo.
2. [ ] Seed the populated demo workspace per runbook §2 — UPDATED recipe: reviewer must be an ACCEPTED identity-bound seat (second Clerk account), one approval made FROM the reviewer account.
3. [~] Rehearse reviewer demo Mode A (live two-account loop, separate browser profiles) per runbook §2a; lock Mode B fallback if the org switch misbehaves twice. The self-accept gap is fixed locally: acceptance now requires a verified invited Clerk email and a non-admin organization member. Pending: deploy this fix, then run the genuine two-account rehearsal. See `docs/DEMO_ACCESS_RUNBOOK.md`.
4. [ ] Decide whether the live claim email is in the demo; if yes, configure Resend + `pinavia.io` sender auth in Render first.
5. [ ] Day-of checklist in runbook §5 within 24h of the demo.

**Weeks 2-5 — launch preparation (launch ~2026-08-04):**
6. [~] Reviewer-seat build — core feature shipped ahead of the 2026-07-21 hard date: migration 0035, invite flow + resend/expiry, scorer gate requires an accepted seat, approval rights restricted to the bound reviewer (`403 approval_requires_bound_reviewer`), and a management page. Local hardening now requires a verified matching Clerk email plus a non-admin organization role before an invite is consumed, preventing self-accept through an admin's secondary email. Pending: deploy and rehearse with two genuine Clerk accounts. See `docs/DEMO_ACCESS_RUNBOOK.md`.
7. [x] Pro waitlist surface — shipped (migration 0037, authed `GET/POST /api/waitlist`, `/pro-waitlist` page, confirmation + ops emails). Follow-up decided 2026-07-09: public Pro-interest capture via the readiness result flow (queued, post-demo).
8. [x] Funnel + pilot-lifecycle panel — shipped (migration-free, `/funnel` + `/api/funnel`), then gated 2026-07-09: `NEXUS_FUNNEL_VISIBILITY` policy (admin default via `NEXUS_OPERATOR_USER_IDS` allowlist), acquisition section operator-only in both modes, nav entry opt-in via `NEXT_PUBLIC_NEXUS_FUNNEL_NAV`.
9. [ ] Public Pro-interest capture on the readiness result (links to workspace waitlist at claim; no anonymous write endpoint). Post-demo.
10. [ ] Low-confidence lane follow-up — onboarding should re-ask sector/size when `laneConfidence` is low instead of silently keeping evaluator.
...
10. [~] Vantage DD Copilot route — launch hub is live at `/vantage` and executable checklist coverage is live at `/vantage/coverage`, using `lib/vantage-dd-workflow.ts` and the governed `vantage_diligence_analysis` runner. Deep deal-room persistence, red-flag workbench, advisor judgment, IC memo, and decision handoff routes remain planned.
...
12. [~] Meridian regulatory route — `/meridian/scope` and `/meridian/license-profile` now persist the shared jurisdiction/regulator/licence/profile record and the profile route cleanly redirects the cold start to Scope. Next: requirement evidence coverage, gap triage, and filing-pack readiness using `lib/domain/regulatory-requirement-library.ts` and `lib/meridian-regulatory-workflow.ts`. Needs domain-reviewed requirements before real customer claims.
13. [~] Nucleus methodology-pack authoring tool — dedicated registry and launch hub are live at `/nucleus`; `/nucleus/profile` now persists the constrained brand layer through workspace settings. Method-pack authoring, validation, versioning, client portal preview, delivery/reviewer controls, and signed-in live save/reload smoke remain planned.
