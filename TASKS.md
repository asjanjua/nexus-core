...
32. [ ] Deploy Commercial Pilot Loop V0.4 — public deploy and smoke are complete for `/`, `/diagnostic`, `/invite/accept?code=missing`, and `/api/health`; anonymous admin/redeem APIs return 401; `/meridian` preserves the sign-in return path. Remaining: set `PINAVIA_ADMIN_PRINCIPALS`, then smoke authenticated `/admin/invites` and a real invite redemption with server-side sample seeding.
