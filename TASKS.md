...
35. [ ] Production-enable the Pinavia trial-invite portal — current `main` is pushed, release-green locally, and the canonical app passed public smoke after a manual latest-commit deploy. `/admin/invites` is now server-gated before its client form renders; its API remains fail-closed. Confirm migration 0038, set `PINAVIA_ADMIN_PRINCIPALS` to Pinavia Clerk staff principals in Render, then complete a real invite → sign-up → redeem → demo-pack smoke. Never put customer-admin principals in this allowlist.
