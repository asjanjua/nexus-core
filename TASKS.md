...
29. [ ] Complete product subdomain DNS cutover — remove the `app.pinavia.io` -> `app.pinavia.co` redirect, add/validate Cloudflare DNS and Render custom domains for `app`, `nexus`, `quorum`, `meridian`, `vantage`, and `nucleus.pinavia.io`, update Clerk allowed origins/redirect URLs, redeploy, and run per-domain smoke before demos. Current evidence is recorded in `docs/PRODUCT_DOMAIN_DNS_CUTOVER_2026-07-26.md`.
...
32. [ ] Deploy Commercial Pilot Loop V0.4 — the full eight-check platform smoke is complete on `app.pinavia.co`; `/`, `/diagnostic`, and `/invite/accept?code=missing` render; anonymous `GET /api/admin/trial-invites` and `POST /api/trial-invites/redeem` return 401; `/meridian` preserves the sign-in return path. `pinavia.io` is the public landing host and intentionally not an API CORS origin. Remaining: set `PINAVIA_ADMIN_PRINCIPALS`, then smoke authenticated `/admin/invites` and a real invite redemption with server-side sample seeding.
...
35. [ ] Production-enable the Pinavia trial-invite portal — push the four local `main` commits, require green CI, apply migration 0038, set `PINAVIA_ADMIN_PRINCIPALS` to Pinavia Clerk staff principals in Render, deploy, then complete a real invite → sign-up → redeem → demo-pack smoke. Never put customer-admin principals in this allowlist.
