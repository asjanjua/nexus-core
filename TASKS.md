...
34. [~] Convert the V0.5 launch order into app changes — Vantage/Nucleus protected route entries are implemented in the current working tree and mirrored in Figma V0.6. Remaining gates: typecheck/build, push, Render deploy, live smoke, then staff invite smoke.
...
10. [~] Vantage DD Copilot route — launch hub exists at `/vantage` using `lib/vantage-dd-workflow.ts`, with dealroom setup, checklist coverage, red flags, IC memo handoff, user inputs, action points, and authority boundaries. Deep deal workspace APIs and `/vantage/*` routes remain planned.
...
13. [~] Nucleus methodology-pack authoring tool — dedicated registry and launch hub exist at `/nucleus`, covering profile, package, delivery, and assurance arcs. Authoring UI, schema, validation, versioning, client portal preview, and deployment controls remain planned.
