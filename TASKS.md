...
3. [~] Rehearse reviewer demo Mode A (live two-account loop, separate browser profiles) per runbook §2a; lock Mode B fallback if the org switch misbehaves twice. UPDATE 2026-07-16: invite + identity-bound accept leg verified live on `app.pinavia.co/reviewer-seat` (seat now shows `accepted` for `ali.janjua@leap-associates.me`, DB-level one-seat constraint confirmed by the UI blocking a second invite). Remaining: an approval made FROM the reviewer account, and the org-switch behavior check — see `docs/agent-runs/2026-07-16/reviewer-seat-two-account-audit-claude.md`.
...
6. [x] Reviewer-seat build — DONE ahead of the 2026-07-21 hard date: migration 0035, invite flow + resend/expiry, accept binds Clerk identity, scorer gate requires accepted seat, approval rights restricted to bound reviewer (`403 approval_requires_bound_reviewer`), management page. Migration-backed management read path is live and clean as of 2026-07-15; invite + identity-bound accept verified live 2026-07-16 (`docs/agent-runs/2026-07-16/reviewer-seat-two-account-audit-claude.md`); the approval-from-reviewer-account leg remains a separate demo gate.
