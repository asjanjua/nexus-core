...
9. [x] Add cron job entries to `render.yaml` -- dispatch runner, billing reset, trial-to-free conversion, and readiness submission pruning. Three cron services added 2026-06-25: dispatch every 2 min, billing daily at midnight, synthesis daily at 1am. Readiness prune added 2026-07-07 at 1:30am. All auth via `NEXUS_CRON_SECRET`. Verify in Render dashboard after deploy.
...
33. [x] Move the Nexus runtime off EOL Node 20 — Node 24 is the production/default target, Node 22.12+ is the explicit compatibility rung, CI verifies both, Render web/cron services pin Node 24, and schema-v2 File Provider caches preserve locked workspace dependencies without cross-major reuse. Node 22 and Node 24 tests/typecheck/163-page builds passed locally on 2026-07-15; remote CI, deploy, and live smoke remain separate gates.
...
1. [~] Release gate — smoke passed on both hosts and SHA confirmed 2026-07-08, BUT local `main` is now 6+ commits ahead again. Remaining: push `main`, apply migrations 0033-0037, set hosted-Clerk + `NEXUS_OPERATOR_USER_IDS` + optional `NEXUS_OPS_EMAIL`/Resend envs, re-smoke. (External: Ali.)
...
6. [x] Reviewer-seat build — DONE ahead of the 2026-07-21 hard date: migration 0035, invite flow + resend/expiry, accept binds Clerk identity, scorer gate requires accepted seat, approval rights restricted to bound reviewer (`403 approval_requires_bound_reviewer`), management page. Production pending: migration 0035 + deploy.
