...
**Diagnostic Intake Honesty Pass (2026-07-27):** `/diagnostic` now avoids implying a live checkout for the diagnostic while the payment/receipt flow is not implemented. The primary CTA reads `Start diagnostic intake` and routes to `/start-pilot?intent=diagnostic`, the secondary path remains `/readiness`, and `/start-pilot` changes its hero copy when diagnostic intent is present. Figma V0.7 frame `Launch Cockpit / 08 Diagnostic Intake Path / 1440` (`145:2`) records the updated handoff and acceptance checks. Local TypeScript, production build, and desktop/mobile route smoke passed; production deploy and live smoke are next.
...
41. [ ] Refresh diagnostic intake path — `/diagnostic` no longer advertises an unwired paid checkout; it routes to `/start-pilot?intent=diagnostic`, keeps `/readiness` as secondary, hides diagnostic pricing until checkout/receipt flows exist, and documents the handoff in Figma frame `145:2`. Local TypeScript, production build, and desktop/mobile route smoke passed; deploy and live smoke pending.
