...
10. [~] Vantage DD Copilot route — launch hub is live at `/vantage` using `lib/vantage-dd-workflow.ts`, with dealroom setup, checklist coverage, red flags, IC memo handoff, user inputs, action points, and authority boundaries. Deep deal workspace APIs and `/vantage/*` routes remain planned.
