...
3. [ ] Rehearse reviewer demo Mode A (live two-account loop, separate browser profiles) per runbook §2a; lock Mode B fallback if the org switch misbehaves twice. CORRECTED 2026-07-16: an invite/accept/approve pass ran on `app.pinavia.co/reviewer-seat` but the audit log shows all three actions (invited, accepted, approved) came from the same Clerk user ID — no second identity was actually involved. Root cause: `accept/route.ts` binds the seat to whoever is signed in when the code is redeemed, without checking the invited email matches that user's own verified emails. Real gap for the demo's separation-of-duties story, not yet fixed. See `docs/agent-runs/2026-07-16/reviewer-seat-two-account-audit-claude.md`, checkpoint `01:15:13`.
...
6. [~] Reviewer-seat build — core feature shipped ahead of the 2026-07-21 hard date: migration 0035, invite flow + resend/expiry, accept binds Clerk identity, scorer gate requires accepted seat, approval rights restricted to bound reviewer (`403 approval_requires_bound_reviewer`, unit-tested and confirmed correct), management page. Migration-backed management read path is live and clean as of 2026-07-15. CORRECTED 2026-07-16: the accept endpoint does not verify the accepting user's email matches the invited email, so self-accept (and therefore self-approval) is currently possible — a real separation-of-duties gap for the regulated-buyer demo. Fix proposed, not yet implemented. See `docs/agent-runs/2026-07-16/reviewer-seat-two-account-audit-claude.md`.
