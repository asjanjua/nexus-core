...
| Pinavia Product Family Final IA V0.3 | 2026-07-25 | Product-family audit and final hub/arc design pass against the locked Pinavia design system | Design-only Figma pass; code implementation pending | Existing Render app unchanged; future implementation should start with NexusAI nav consolidation, then Quorum, Meridian, Vantage, Nucleus | Audit `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=101-30`; IA reference `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=101-97`; NexusAI `101:158`; Quorum `101:329`; Meridian `101:517`; Vantage `101:677`; Nucleus `101:837` | candidate | Added final product-family pages with three-tier IA, hub skeleton, state strips, arc landings, and Nucleus Engagement Room + 8 greenfield screens. Follow-up exact-change review fix rebuilt those eight Nucleus screens as full 1440x900 frames and expanded Audit Findings to 12 pre-ship checks. Existing boards `78:3`, `80:3`, and `87:3` were preserved and marked with reconciliation notes. Added local Pinavia token/text styles for easier future edits. |
| Commercial Pilot Loop V0.4 | 2026-07-26 | Code-backed visual pass from the current local commercial tree: Pinavia landing, priced diagnostic, Meridian Submission Room, and trial-invite portal | `ba3e440` plus the pushed product stack behind it | `https://app.pinavia.io` passed the full eight-check platform smoke on 2026-07-26; `/`, `/diagnostic`, and `/invite/accept?code=missing` render; `/meridian` redirects to sign-in. `https://pinavia.io` remains the public landing host and intentionally is not an API CORS origin. `/admin/invites` requires `PINAVIA_ADMIN_PRINCIPALS` before full staff smoke. | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=121-2` | candidate | Six 1440x900 desktop-browser frames: landing CTA delta, two-rung diagnostic, Meridian code-backed hub, staff invite portal, invite accept/redeem, and commercial pilot storyboard. See `docs/UI_V0_4_COMMERCIAL_PILOT_LOOP_FIGMA_PLAN.md`. |
| Initial Launch Final Pack V0.5 | 2026-07-26 | Launch-lock board over all current final UI pages and live route evidence | Design-only coordination board created after `72cff26`; code changes follow the screen source map | Public launch routes from V0.4 remain live; Vantage and Nucleus are explicitly design-only until routes exist | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=125-2` | candidate | Six 1440x900 desktop-browser coordination frames: readiness matrix, core buyer path, pilot surface lock, screen handoff index, gaps before pilot demo, and ten-minute final demo script. See `docs/UI_V0_5_INITIAL_LAUNCH_FINAL_PACK.md`. |
| Launch Route Update V0.6 | 2026-07-26 | Code-backed route-entry update after V0.5: Vantage and Nucleus protected hubs now exist | `20f2848` plus live-smoke docs follow-up | `https://pinavia.io/vantage` and `/nucleus` redirect signed-out users to sign-in with return paths; `https://pinavia.io/api/health` returned `ok=true` after deploy. Deep `/vantage/*` and `/nucleus/*` route trees remain planned until implemented and smoked | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=126-2` | candidate | Three 1440x900 desktop-browser frames: readiness delta, Vantage/Nucleus route hubs, and updated demo script. See `docs/UI_V0_6_LAUNCH_ROUTE_UPDATE.md`. |
| Initial Launch Cockpit V0.7 | 2026-07-26 | Screenshot-verified colleague review cockpit that consolidates V0.4, V0.5, V0.6, live-route truth, DNS blockers, and next actions | `f1bfa46` plus `f9de6b2` public Solutions/Research pass and end-to-end pilot design closure | Apex route smoke remains the demo path: `https://pinavia.io`, `/solutions`, `/research`, `/readiness`, `/diagnostic`, `/vantage`, `/nucleus`, `/start-pilot`, and `/product-brief`. `f9de6b2` live apex smoke confirms `/solutions` and `/research` both return `200` with expected launch copy and `/api/health` is `ok`. Meridian scope/profile, Vantage coverage, Nucleus firm profile, and connector intake are code-backed protected routes; signed-in live smoke remains due. The six end-to-end handoff frames are design-final planned deep routes, not live claims. Product subdomains remain blocked until DNS/Render/Clerk cutover passes | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=129-6`; pilot-start `135:2`; product-brief `141:2`; readiness-result `143:2`; diagnostic-intake `145:2`; Meridian scope arc `160:2`; Vantage coverage `162:2`; Nucleus profile `164:2`; NexusAI Ask `167:2`; Quorum board loop `167:57`; connector intake `173:2`; solutions `176:2`; research `176:63`; end-to-end handoffs `182:2`, `182:74`, `182:146`, `182:218`, `182:290`, `182:362` | candidate | Twenty-two 1440x900 desktop-browser frames. The current additions make the full pilot handoff stories, public solutions/research, product entry slices, and governed connector intake editable and reviewable. See `docs/UI_V0_7_INITIAL_LAUNCH_COCKPIT.md`. |
| Pinavia Product Front Doors V0.8 | 2026-07-29 | Simplified Pinavia corporate home and branded product-entry layer | Design-only Figma pass; implementation pending | Existing `pinavia.io` / `app.pinavia.io` remain live. `nexus`, `quorum`, `meridian`, `vantage`, and `nucleus.pinavia.io` now use verified Cloudflare 301 entry redirects to canonical app routes; they are not separate authenticated origins. | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=187-2`; dark home `188:2`; dark menu `190:2`; dark NexusAI `191:2`; dark Quorum `191:38`; dark Meridian `191:74`; dark Vantage `191:110`; dark Nucleus `191:146`; day home `196:11`; day menu `197:2`; day NexusAI `197:45`; day Quorum `197:81`; day Meridian `197:117`; day Vantage `197:153`; day Nucleus `197:189`; About `192:2`; Leap team `192:32` | candidate | Sixteen editable 1440x900 desktop frames: the original seven dark product-entry references, seven Day-mode operational references, About Pinavia, and a content-ready Leap Associates team page. `Pinavia Theme` has semantic Dark and Day color modes in Figma; existing approved paint styles remain intact until the code and design system are migrated deliberately. The front doors preserve the distinction between branded entry URLs and the shared authenticated Pinavia app. |
| Pinavia Site Flow + Design Audit V0.9 | 2026-07-29 | Dated executive review of the public site, conversion path, product-entry layer, and next design actions | Design-only Figma audit; no live-status changes implied | The audit distinguishes live public/app routes from design-ready front doors and flags mobile/accessibility and commercial proof as work still requiring production evidence. | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=200-2`; site flow `201:2`; scorecard `203:2`; action ledger `205:2` | candidate | Three editable 1440x900 desktop audit frames. The site-flow map records visitor-to-product routing and shared-app boundaries; the scorecard separates Ready, Designed, and Needs proof; the action ledger orders work and makes the release gate explicit. |
| Pinavia Code-to-Visual Tree V1.0 | 2026-07-29 | Dated technical visual map of the Pinavia product tree, deployment envelope, Git provenance, and code-to-Figma boundaries | Design-only Figma architecture artefact; it records current code and design status but does not certify routes beyond existing smoke evidence | The map uses green for live/code-backed surfaces, violet for designed next-state workflow arcs, blue for external services, and product-specific accents for NexusAI, Quorum, Meridian, Vantage, and Nucleus. | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=206-2`; product/route forks `207:2`; code/deployment boundaries `208:2`; Git-to-visual provenance `209:2` | candidate | Three detailed 2400px-wide technical boards. They show the shared Next.js core, public-to-app routing, Cloudflare/Render/Clerk/Postgres/R2/Stripe/Resend/Plausible boundaries, pivot-specific registries and routes, design-only workflow arcs, feature-branch provenance, and the required chain from design reference to live claim. |
| Nexus Room Portfolio V1.1 | 2026-07-29 | Day-one visible room portfolio and administrator-governed activation model for NexusAI, extended into product-room discovery | Design-only Figma pass; implementation pending | Current onboarding role suggestion, staged/dual-hat roles, and custom role titles are code-backed. Durable room configuration, `/rooms`, configuration-derived navigation, and product activation adapters remain planned. | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=212-2`; portfolio `213:2`; first-run `213:61`; template `214:2`; configure `214:59`; review `214:114`; Finance `215:2`; product portfolio `215:49`; Executive `218:2`; Operating `218:49`; Growth `218:96`; Technology and Data `218:143`; Risk `218:190`; People `218:237`; Quorum `218:284`; Meridian `218:331`; Vantage `218:378`; Nucleus `218:425`; staged/dual-hat `218:472`; custom `218:527`; tree branch `216:2` | candidate | Nineteen editable 1440x900 Nexus frames plus a 2400px-wide architecture branch. The set now includes a dedicated titled screen for every leadership and product room, plus all portfolio and activation states. Every workspace sees the full portfolio from day one; activation requires a workspace administrator, named owner, evidence scope or empty-state reason, agent boundary, human-authority acknowledgement, and audit event. Quorum, Meridian, Vantage, and Nucleus stay separate vertical workflows. |
| Vertical Product Screen Plans V1.2 | 2026-07-29 | Product-owned screen architecture, full Quorum/Meridian/Vantage/Nucleus desktop workflow sets, and shared trust/failure-state plan | Design-only Figma pass; route registry and enforcement contract are documented, code implementation pending | Existing hub routes may be code-backed; all candidate deep routes remain planned until their persistence, permissions, audit events, tests, and production smoke are complete. | Plan `222:2`; Quorum `80:3`; Meridian `226:2`; Vantage `230:2`; Nucleus `232:2`; trust/failure states `233:2` | candidate | The vertical route registry is `docs/VERTICAL_PRODUCT_SCREEN_PLANS_2026-07-29.md`; do not duplicate vertical candidate routes in this ledger. The set includes 53 primary workflow screens and 12 explicitly-planned trust/failure states. See `docs/VERTICAL_PRODUCT_TRUST_AND_FAILURE_CONTRACT.md`. |
...
The V0.7 cockpit now contains twenty-two 1440x900 desktop-browser frames. `Launch Cockpit / 11 Nucleus Firm Profile / 1440` (`164:2`) maps the code-backed `/nucleus/profile` route: a constrained firm brand layer (name, logo URL, accent, typeface) and an adjacent contract panel that keeps provenance, status semantics, approval boundaries, audit labels, and consequence previews fixed. `5e8e7d1` is live: focused tests, TypeScript, production build, signed-out Clerk return-path smoke, and `/api/health` pass. A signed-in safe brand save/reload remains pending. Frames `167:2` and `167:57` put the code-backed NexusAI and Quorum core pilot loops on the same editable launch board. Frame `173:2` adds the code-backed connector first-run path. Frames `176:2` and `176:63` add standalone Pinavia Solutions and Research public surfaces. Frames `182:2` through `182:362` add six design-final end-to-end pilot handoffs; their deep routes remain planned until implemented and smoked.
...
| Vertical product screen plans V1.2 | `30 Vertical Product Screen Plans / 2026-07-29`, board `222:2` | Editable Figma frames | Four product-owned architecture plans: Quorum (17 screens), Meridian (12), Vantage (12), Nucleus (12). | Design and delivery plan only; candidate routes must not be described as live until implemented and verified. |
...
| Product vertical hubs | `/board`, `/meridian`, `/vantage`, `/nucleus` |
| Vertical candidate-route registry | `docs/VERTICAL_PRODUCT_SCREEN_PLANS_2026-07-29.md` |
