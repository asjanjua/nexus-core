...
| Initial Launch Cockpit V0.7 | 2026-07-26 | Screenshot-verified colleague review cockpit that consolidates V0.4, V0.5, V0.6, live-route truth, DNS blockers, and next actions | `f1bfa46` plus follow-up commits through `914b71f` | Apex route smoke remains the demo path: `https://pinavia.io`, `/readiness`, `/diagnostic`, `/vantage`, `/nucleus`, `/start-pilot`, and `/product-brief`. `/start-pilot` live smoke returns 200 with the expected Pinavia shell and workspace CTA; `/` live smoke confirms the homepage/header CTA convergence on `/start-pilot`; `/product-brief` live smoke passes desktop/mobile with no stale NexusAI-only strings, three `/start-pilot` links, all product-room links, no mobile overflow, desktop table retained, and five mobile room cards. `/readiness` live result-flow smoke passes desktop/mobile with Pinavia copy, inherited signup, `/start-pilot`, no stale personal-email/NexusAI-only copy, no contradictory advisor panel, and no overflow. `/diagnostic` live smoke passes desktop/mobile with no unwired checkout claim, primary handoff to `/start-pilot?intent=diagnostic`, `/readiness` as secondary, diagnostic-intent start page copy, and no overflow. Meridian scope/profile, Vantage coverage, Nucleus firm profile, and connector intake are code-backed protected routes; signed-in live smoke remains due. Product subdomains remain blocked until DNS/Render/Clerk cutover passes | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=129-6`; pilot-start `135:2`; product-brief `141:2`; readiness-result `143:2`; diagnostic-intake `145:2`; Meridian scope arc `160:2`; Vantage coverage `162:2`; Nucleus profile `164:2`; NexusAI Ask `167:2`; Quorum board loop `167:57`; connector intake `173:2` | candidate | Fourteen 1440x900 desktop-browser frames. The current additions make the Nucleus firm-profile, NexusAI Ask, Quorum board-action, and governed connector intake loops editable and reviewable. See `docs/UI_V0_7_INITIAL_LAUNCH_COCKPIT.md`. |
