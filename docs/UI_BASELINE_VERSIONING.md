...
| Initial Launch Cockpit V0.7 | 2026-07-26 | Screenshot-verified colleague review cockpit that consolidates V0.4, V0.5, V0.6, live-route truth, DNS blockers, and next actions | `f1bfa46` plus follow-up commits through `d1ac9b2` | Apex route smoke remains the demo path: `https://pinavia.io`, `/readiness`, `/diagnostic`, `/vantage`, `/nucleus`, `/start-pilot`, and `/product-brief`. `/start-pilot` live smoke returns 200 with the expected Pinavia shell and workspace CTA; `/` live smoke confirms the homepage/header CTA convergence on `/start-pilot`; `/product-brief` live smoke passes desktop/mobile with no stale NexusAI-only strings, three `/start-pilot` links, all product-room links, no mobile overflow, desktop table retained, and five mobile room cards. `/readiness` live result-flow smoke passes desktop/mobile with Pinavia copy, inherited signup, `/start-pilot`, no stale personal-email/NexusAI-only copy, no contradictory advisor panel, and no overflow. Product subdomains remain blocked until DNS/Render/Clerk cutover passes | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=129-6`; pilot-start frame `135:2`; product-brief frame `141:2`; readiness-result frame `143:2` | candidate | Seven 1440x900 desktop-browser frames: executive launch status, demo route map, final screen QA matrix, action board, pilot-start intake, product-family brief, and readiness-result path. Frame `135:2` records the homepage/header CTA convergence on `/start-pilot`; frame `141:2` records the `/product-brief` refresh; frame `143:2` records `/readiness` result CTAs, inherited-result signup, no stale copy, and no contradictory advisor state. See `docs/UI_V0_7_INITIAL_LAUNCH_COCKPIT.md`. |
