...
| Initial Launch Cockpit V0.7 | 2026-07-26 | Screenshot-verified colleague review cockpit that consolidates V0.4, V0.5, V0.6, live-route truth, DNS blockers, and next actions | `f1bfa46` plus follow-up commits through `1b8c5f4`; homepage CTA alignment pending deploy | Apex route smoke remains the demo path: `https://pinavia.io`, `/diagnostic`, `/vantage`, `/nucleus`, and `/start-pilot`. `/start-pilot` live smoke returns 200 with the expected Pinavia shell and workspace CTA. Product subdomains remain blocked until DNS/Render/Clerk cutover passes | `https://www.figma.com/design/NcQ8F5a0hczwGwZua2gfun?node-id=129-6`; pilot-start frame `135:2` | candidate | Five 1440x900 desktop-browser frames: executive launch status, demo route map, final screen QA matrix, action board, and pilot-start intake. Frame `135:2` now also records the homepage/header CTA convergence on `/start-pilot`. See `docs/UI_V0_7_INITIAL_LAUNCH_COCKPIT.md`. |
