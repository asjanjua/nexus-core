...
| **Quorum** | "Board and committee cycles: packs, minutes, resolutions, follow-through" | ◐ The current runtime includes `/board`, `POST /api/board/delta`, and the authenticated `POST /api/agents/native-skills/quorum-governance-review` path. The governance review reads processed workspace evidence, decisions, and actions; returns evidence-cited completeness findings, decision/action gaps, a human approval packet, and governance caveats; and records started/completed audit events. It does **not** generate a statutory board pack, minutes, circular resolutions, committee registers, calendar, signature workflow, or final record. The 17-screen Figma and code registry remain the planned lifecycle surface. |
...
**Risk level: HIGH.** A client who asks for a complete board portal will not find one. The demonstrable Quorum claim is narrower: an authenticated, evidence-cited governance review that identifies board-pack gaps and decision/action follow-through while keeping human approval and statutory responsibility outside the product. Keep Quorum's broader lifecycle screens in "In development" or "Roadmap" until a controlled board-cycle proof and the relevant product surfaces exist.
