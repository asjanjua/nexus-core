...
Frames seventeen through twenty-two close the design-story gap inside each pilot. Meridian evidence/gaps and filing, Vantage red-flag/IC handoff, Nucleus partner review/client preview, and Quorum minutes/action review now have protected review routes; NexusAI continues to use the existing `/approvals` surface. Each new route is an interactive review workbench with named owner, target date, reviewer-note inputs, and a prefilled handoff into the existing human-owned Decisions flow. It does not persist a regulatory, board, investment, or client conclusion. All six frames were screenshot-reviewed; title wraps in Quorum and NexusAI and a too-narrow Vantage status chip were corrected before registration.
...
The connector pass deployed through `96fcd8f`. CI passed, the live `/settings/connectors` response contains the new intake/reference copy, and `/api/health` remained `ok`. A follow-up route refactor deployed through `9747003` makes the page genuinely workspace-authenticated before the client catalogue renders; the frame chrome now says `Authenticated workspace`, and signed-out live smoke returns the exact Clerk return path. This proves the released UI and entry boundary, not a provider consent, policy save, or ingest outcome; those remain signed-in controlled-workspace checks.
