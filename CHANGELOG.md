...
- Updated product-hub CTAs to reach the new review routes. Local TypeScript and production build pass; `cf2c2a4` live signed-out return-path smoke and health check pass.
...
- Deployed the Vantage Coverage slice through `f357ace`; live signed-out smoke confirms the exact Clerk return path for `/vantage/coverage`, and `https://pinavia.io/api/health` remains healthy. A signed-in evidence/run/result smoke is the remaining pilot proof.
